...
<h3 class="w3-panel w3-blue w3-center w3-round-xxlarge">SET ACCESS CONTROL</h3><hr>
<%
...
rs=stmt.executeQuery("select * from members where gid ='"+ tgid + "' and usrid<>" + tid);
out.println("<form method=post action='acess3.jsp'>");
out.println("<table ALIGN=CENTER><tr><td>Group ID</td><td><input type=text size=10 name=gid value=" + tgid + "></td></tr>");
out.println("<tr><td>User ID</td><td><select name=usid style='width:120px;'>");
while(rs.next())
{
out.println("<option>" + rs.getString(2) + "</option>");
}
out.println("</select></td></tr>");
out.println("<tr><td></td><td><input type=submit value='Disallow'></td></tr></table></form>");
rs.close();
