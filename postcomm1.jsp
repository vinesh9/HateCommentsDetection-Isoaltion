...
out.println("<form method=post action='postcomm2.jsp'>");
// select gid from members where usrid=tid and status='Allow'
rs=stmt.executeQuery("select bid,btitle,comm from blogs where gid in(select gid from members where usrid=" + tid + ")");
out.println("<table class='w3-table-all  w3-centered'><tr class='w3-blue'><td><b>Blog ID</td><td><b>Title</td><td><b>Comment Posted</td></tr>");
while(rs.next())
{
String tbid=rs.getString(1);
String title=rs.getString(2);
String comm=rs.getString(3);
out.println("<tr><td>" + tbid + "</td><td><a href='postcomm2.jsp?btitle="+title+"&bid="+tbid+"' method=post>" + title + "</a></td><td>" + comm + "</td></tr>");
}
rs.close();
rs=stmt.executeQuery("select bid,btitle,comm from blogs where usrid ="+ tid + " and gid not in(select gid from members where usrid=" + tid + ")");
while(rs.next())
{
String tbid=rs.getString(1);
String title=rs.getString(2);
