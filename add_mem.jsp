...
<h3 class="w3-panel w3-blue w3-center w3-round-xxlarge">Add Members</h3><hr>
<%
...
out.println("<form method=post action='add_mem2.jsp'>");
out.println("<table align=center class='w3-table-all w3-centered'><tr><td>Select Group ID</td><td><select name=gid>");
while(rs.next())
{
...
rs=stmt.executeQuery("select * from userregn where usrid <>"+ tid);
out.println("<tr><td>Select User ID</td><td><select name=usid>");
while(rs.next())
{
out.println("<option>" + rs.getString(1) + "</option>");
}
out.println("</select></td></tr>");
out.println("<tr><td></td><td><input type=submit value='Add member'></td></tr></table></form>");
rs.close();
