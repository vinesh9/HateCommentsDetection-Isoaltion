...
<h3 class="w3-panel w3-blue w3-center w3-round-xxlarge">New Blog</h3><hr>
<%
usid=(String)session.getAttribute("slogid");
out.println("<form method=post action='add_blog.jsp' class='w3-panel'>");
out.println("<table class='w3-container' align=center>");
out.println("<tr><td>Blog Title</td><td><input type=text name=btitle size=15 required=required></td></tr>");
out.println("<tr><td>Blog Info</td><td><textarea name=bcomm rows=10 cols=50 required=required></textarea></td></tr>");
try
{
Class.forName("oracle.jdbc.driver.OracleDriver");
con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","comments","comments");
stmt1=con.createStatement();
out.println("<tr><td>Group ID</td><td><select name=gid required=required>");
rs=stmt1.executeQuery("select gid from groups where ownid=" + usid);
while(rs.next())
{
...
out.println("<tr><td>Filter Mode</td><td><input type=radio name=fmode value='Manual' checked> Manual <input type=radio name=fmode value='Automatic'> Automatic</td></tr>");
rs.close();
}
catch(Exception ee)
{
out.println("<b>Group with this ID already Exists!");
}
con.close();
out.println("<tr><td></td><td><input type=submit value='Save'><input type=reset></td></tr>");
